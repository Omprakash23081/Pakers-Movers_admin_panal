        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 self-start">
            <button
              ...
              className={`px-6 py-2.5 rounded-lg text-sm font-black tracking-tight transition-all ${activeTab === 'active' ? 'bg-primary text-white shadow-xl shadow-primary/20 scale-105' : 'text-secondary hover:text-white'}`}
            >
              Approved Leads ({categorizedClients.active.length})
            </button>
            <button
              onClick={() => setActiveTab('shipped')}
              className={`px-6 py-2.5 rounded-lg text-sm font-black tracking-tight transition-all ${activeTab === 'shipped' ? 'bg-primary text-white shadow-xl shadow-primary/20 scale-105' : 'text-secondary hover:text-white'}`}
            >
              Shipped ({categorizedClients.shipped.length})
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:hidden">
          {filteredLeads.length > 0 ? (
            filteredLeads.map((lead) => (
              <div key={lead._id} className="apple-card p-5 border-white/10 space-y-4">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider border mt-1 ${
                        lead.status === 'shipped' 
                        ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' 
                        : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'
                      }`}>
                        {lead.status === 'shipped' ? 'Shipped' : 'Approved'}
                      </span>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-white/5 text-secondary border border-white/10"
                    >
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="flex items-center gap-3 text-sm text-secondary">
                    ...
                    <span>{lead.movingFrom} → {lead.movingTo}</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-secondary">
                    <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center">
                      <Calendar className="w-4 h-4 text-primary" />
                    </div>
                    <span>Approved: {new Date(lead.updatedAt).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
            ))
             ...
             <div className="p-10 text-center bg-white/[0.02] rounded-3xl border border-dashed border-white/10">
               <Users className="w-10 h-10 mx-auto text-secondary opacity-20 mb-3" />
               <p className="text-secondary font-bold">No leads found.</p>
             </div>
          )}
        </div>

        {/* Desktop View: Table Layout */}
        <div className="hidden md:block apple-card overflow-hidden border-white/10">
            ...
            <table className="w-full text-left whitespace-nowrap">
              <thead>
                <tr className="border-b border-white/5 bg-white/[0.02]">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-secondary uppercase tracking-wider">Approved On</th>
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-3 text-secondary">
                        ...
                        <p className="text-lg font-medium opacity-60">No approved leads found yet.</p>
                        <p className="text-sm opacity-40 max-w-xs mx-auto">When you update an inquiry status to "Approved" in the Quote Requests section, they will appear here automatically.</p>
                      </div>
                    </td>
                  </tr>
                )}
